...
const makeWavetable = function (ac) {
    return (function (wavedata) {
        ...
        piano: {
            ...
            defs: { wave: "piano", gain: 0.5, attack: 0.002, decay: 0.25, sustain: 0.03, release: 0.1, decayfollow: 0.7, cutoff: 800, cutfollow: 0.1, resonance: 1, detune: 0.9994 },
        },
    });
};
...
class Utils {
    ...
    static resetAudio() {
        if (Utils._audioTop) {
            ...
            dcn.attack.setValueAtTime(0.0005, 0);
            // dcn.ratio = 16; // RONYEH: .ratio is a READ ONLY property.
    ...
    // Some constants.
    static noteNum = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11, c: 12, d: 14, e: 16, f: 17, g: 19, a: 21, b: 23 };
    static accSym = { "^": 1, "": 0, "=": 0, _: -1 };
    static noteName = ["C", "^C", "D", "_E", "E", "F", "^F", "G", "_A", "A", "_B", "B", "c", "^c", "d", "_e", "e", "f", "^f", "g", "_a", "a", "_b", "b"];
    ...
    static makeTimbre(options, atop) {
        if (!options) {
            options = {};
        }
    ...
    static whiteNoiseBuf = null; // cache
    static getWhiteNoiseBuf() {
    ...
    // defined right below.
    static makeOscillator(atop, wavename, freq) {
        if (wavename == "noise") {
...
class Instrument {
    static timeOffset = 0.0625; // Seconds to delay all audiable timing.
    static dequeueTime = 0.5; // Seconds before an event to reexamine queue.
    static bufferSecs = 2; // Seconds ahead to put notes in WebAudio.
    static toneLength = 1; // Default duration of a tone.
    static cleanupDelay = 0.1; // Silent time before disconnecting nodes.

    // Accepts either an ABC pitch or a midi number and converts to midi.
    static pitchToMidi(n) {
        if (typeof n == "string") {
    ...
    silence() {
        ...
        // this._out.gain.value = initvolume; // RONYEH REMOVED DUE TO BUG.
    ...
    _makeSound(record) {
        ...
        if (record.duration > 0 && record.velocity > 0) {
            ...
            g.gain.setTargetAtTime(amp * timbre.sustain, decaystarttime, decaytime);
            ...
            if ((!timbre.cutoff && !timbre.cutfollow) || timbre.cutoff == Infinity) {
                ...
                // f.frequency.value = timbre.cutoff + record.frequency * timbre.cutfollow; // RONYEH REMOVED DUE TO BUG.
    ...
    _truncateSound(record, truncatetime) {
    ...
    _startPollTimer(setnow: boolean = false) {
    ...
    tone(pitch, duration, velocity, delay, timbre, origin) {
    ...
    play(abcstring) {
        ...
        for (k = 0; k < files.length; ++k) {
            ...
            for (vn in abcfile.voice) {
                ...
                timbre = Utils.makeTimbre(opts["timbre"] || abcfile.voice[vn].timbre || abcfile.timbre || this._timbre, this._atop);
...
export default { Instrument, parseABCFile };
