...
const TIME_THRESHOLD_FOR_GROUPING_NEARBY_NOTES = 0;
...
var MIDIPianoInput;
(function (MIDIPianoInput) {
    let numNotesPressed = 0;
    function setup() {
...
function addTracks(numTracks) {
    ...
    for (let t = 0; t < numTracks; t++) {
        ...
        html += `<div id="track-${t}-container" class="track-container">${checkbox}${info}${track}</div></div>`;
    }
    $('#tracks').html(html);
    for (let t = 0; t < numTracks; t++) {
...
(function (LocalStorage) {
    ...
    function loadCheckboxes() {
        try {
            let savedCheckboxState = JSON.parse(localStorage.getItem('checkboxes'));
...
function playOneNote(pianoKeyBeforeModifiers, applyModifiers = true) {
...
(function (MIDI) {
    ...
    function getFileFromTracks() {
        ...
        let noteGroupsToTurnOff = new Map();
        ...
        for (let currNoteGroup of noteGroups) {
            ...
            if (!midiTrack) {
                console.log('OOPS: MIDI TRACK IS NULL');
                continue;
            }
            ...
            currNoteGroup.notes.forEach((note, index) => {
                midiTrack.noteOn(CHANNEL, note.midiNote, 0, note.velocity);
            });
        ...
        for (let trackNumber in noteGroupsToTurnOff) {
            ...
            noteGroupToTurnOff.notes.forEach((note, index) => {
                let deltaTimeTicks = (index === 0) ? TICKS_PER_SECOND : 0;
    ...
    function parseData(arrayBuffer) {
        ...
        let lastMidiEvent = midiEvents[midiEvents.length - 1];
    ...
    function fillTracksWithNoteGroups() {
        ...
        for (let event of midiEvents) {
            ...
            let noteToPlay = new Note(pianoNoteNum, 1.0, velocity);
...
(function (Playback) {
    ...
    function playNextEvents(currTime) {
        ...
        while (currSongTime >= nextEventPlayTime) {
            let noteGroup = noteGroupsToPlay[currNoteGroupNumber];
    ...
    function determinePlayTimeForNextEvent() {
        ...
        if (nextEventPlayTime === -1) {
            nextEventPlayTime = currSongTime + TIME_BETWEEN_NOTEGROUPS;
        }
    }
    ...
    Playback.playNoteAndGoBackwardInTheSong = _.throttle(function () {
        playNoteAndAdvanceInDirection(-1);
    }, 150);
    Playback.playNoteAndGoForwardInTheSong = _.throttle(function () {
        playNoteAndAdvanceInDirection(+1);
    }, 150);
    ...
    Playback.playAndGoBackwardOnCurrentTrack = _.throttle(function () {
        playCurrentNoteGroup();
        Highlight.prevNoteGroup();
    }, 150);
    Playback.playAndGoForwardOnCurrentTrack = _.throttle(function () {
        playCurrentNoteGroup();
...
(function (UI) {
    ...
    function showNoteGroupsForTracks() {
        ...
        for (let t = 0; t < numTracks; t++) {
            ...
            for (; n < numNoteGroups; n++) {
                ...
                let noteGroupID = getNoteGroupID(t, n);
    ...
    function onKeyDownHandler(e) {
        ...
        switch (keyCode) {
            ...
            case 34:
                ...
                break;
            case 36:
                Playback.playNoteAndGoBackwardInTheSong();
                break;
            case 35:
                Playback.playNoteAndGoForwardInTheSong();
                break;
            case 112:
                console.log('F1');
                break;
    ...
    function drawMostRecentGroup(c) {
        ...
        for (let n of notes) {
            ...
            c.fillStyle = 'rgba(220,220,10,.82)';
            c.fill();
        }
    }
    function setupPianoMouseHandlers() {
        const whiteKeyOffsets = [1, 3, 4, 6, 8, 9, 11];
        const blackKeyOffsets = [2, 0, 5, 7, 0, 10, 12];
...
(function (Song) {
    ...
    function getNoteGroupsFromTracks() {
        ...
        let noteGroups = [];
        let currTimeMillis = 0;
        let trackIsChecked = [];
        let noteGroupNumberPerTrack = [];
        let numTracks = tracks.length;
        for (let t = 0; t < numTracks; t++) {
