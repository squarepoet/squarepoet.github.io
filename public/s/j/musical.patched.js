            ...
            2: [
                function (require, module, exports) {
                    ...
                    Instrument.prototype.silence = function () {
                        ...
                        // this._out.gain.value = initvolume; // RONYEH REMOVED
                        ...
                        if (record.duration > 0 && record.velocity > 0) {
                            ...
                            while (decaystarttime < attacktime + 1 / 32 && decaystarttime + 1 / 256 < releasetime) {
                                ...
                                g.gain.linearRampToValueAtTime(
                                    amp * (timbre.sustain + (1 - timbre.sustain) * Math.exp((attacktime - decaystarttime) / decaytime)),
                                    decaystarttime
                                );
                            }
                            ...
                            g.gain.setValueAtTime(
                                amp * (timbre.sustain + (1 - timbre.sustain) * Math.exp((attacktime - releasetime) / decaytime)),
                                releasetime
                            );
                            ...
                            if ((!timbre.cutoff && !timbre.cutfollow) || timbre.cutoff == Infinity) {
                                ...
                                // f.frequency.value = timbre.cutoff + record.frequency * timbre.cutfollow; // RONYEH REMOVED
                                // f.Q.value = timbre.resonance; // RONYEH REMOVED
                                f.frequency.setTargetAtTime(
                                    timbre.cutoff + record.frequency * timbre.cutfollow,
                                    this._atop.ac.currentTime,
                                    0.001
                                ); // RONYEH ADDED
                            ...
                            if (doubled) {
                                o2 = makeOscillator(this._atop, timbre.wave, record.frequency * timbre.detune);
                                o2.connect(f);
                                o2.start(starttime);
                                o2.stop(stoptime);
                            }
                        ...
                        if (truncatetime < record.time + record.duration) {
                            ...
                            if (record.gainNode) {
                                ...
                                if (releasetime <= starttime) {
                                    ...
                                    g.gain.linearRampToValueAtTime((amp * (releasetime - starttime)) / (attacktime - starttime), starttime); // RONYEH ADDED SECOND PARAM
                                } else {
                        ...
                        for (j = 0; j < this._startSet.length; ++j) {
                            if (this._startSet[j].time <= instant) {
                                ...
                                if (this._finishSet.hasOwnProperty(freq)) {
                                    ...
                                    if (
                                        conflict.time < record.time ||
                                        (conflict.time == record.time && conflict.duration < record.duration)
                                    ) {
                    ...
                    Instrument.prototype.play = function (abcstring) {
                        ...
                        for (k = 0; k < files.length; ++k) {
                            ...
                            for (vn in abcfile.voice) {
                                ...
                                timbre = makeTimbre(opts.timbre || abcfile.voice[vn].timbre || abcfile.timbre || this._timbre, this._atop);
            ...
            4: [
                function (require, module, exports) {
                    (function (global) {
                        ...
                        // Some constants.
                        var noteNum = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11, c: 12, d: 14, e: 16, f: 17, g: 19, a: 21, b: 23 };
                        var accSym = { "^": 1, "": 0, "=": 0, _: -1 };
                        var noteName = [
                            "C",
                            "^C",
                            "D",
                        ...
                        module.exports.pitchToMidi = pitchToMidi = function (pitch) {
                            ...
                            var semitone = noteNum[m[2]] + accSym[m[1].charAt(0)] * m[1].length + 12 * octave;
                        ...
                        module.exports.makeOscillator = makeOscillator = function (atop, wavename, freq) {
                            ...
                            // o.frequency.value = freq; // RONYEH REMOVED
